...
export default function AdminServicesPage() {
  ...
  const handleUploadImage = async () => {
    ...
    try {
      ...
      const res = await fetch("http://14.187.180.6:12122/api/images/upload", {
        method: "POST",
        headers: { Authorization: `Bearer ${token}` },
        body: fd
      })
  ...
  const handleUpdateImage = async () => {
    ...
    try {
      const res = await fetch(`http://14.187.180.6:12122/api/images/${selectedImage.id}`, {
        method: "PUT",
        headers: {
  ...
  const handleDeleteImage = async (img: ImageItem) => {
    ...
    try {
      const res = await fetch(`http://14.187.180.6:12122/api/images/${img.id}`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` }
      })
      if (res.ok) {
